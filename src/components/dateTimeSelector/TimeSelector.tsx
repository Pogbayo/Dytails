    ...
    unavailableTimes.forEach((time) => {
      ...
      for (let i = -3; i <= 3; i++) {
        ...
        if (adjustedHour >= 0 && adjustedHour < 24) {
          const formattedTime = `${String(adjustedHour).padStart(
            2,
            "0"
    ...
    <div className=" pt-4 pb-4 md:pb-0 flex flex-col items-center max-h-96 ">
      ...
      <div className="max-h-72 overflow-auto flex flex-col items-center w-screen pr-4 pl-4 md:w-full">
        {availableTimes.map((time, i) => (
          <button
            className={`bg-royalblue md:bg-darkblue p-4 w-full  md:w-64 h-12 rounded-2xl flex flex-col items-center justify-center mt-1 mb-1 ${
              time == selectedTime ? "bg-teal md:bg-teal" : ""
            }`}
            key={i}
